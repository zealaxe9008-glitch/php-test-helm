...
What this Helm command does:
- `helm upgrade --install`:
  - Install if the release doesn’t exist; otherwise apply changes to the existing release.
- `laravel`:
  - The release name (you’ll see Kubernetes objects prefixed with it).
- `./deploy/helm/laravel-app`:
  - Path to the chart to install.
- `-n laravel`:
  - Target namespace for all resources.
- `--set image.repository=phpapp` and `--set image.tag=apache`:
  - Tell the chart which image and tag to deploy.
- `--set ingress.enabled=true`:
  - Create an Ingress resource.
- `--set ingress.className=nginx`:
  - Use the NGINX ingress controller (minikube addon).
- `--set ingress.hosts[0].host=laravel.local`:
  - The hostname you’ll use in your browser.
- `--set ingress.hosts[0].paths[0].path=/` and `pathType=Prefix`:
  - Route all requests under `/` to this Service.
